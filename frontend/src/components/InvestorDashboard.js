...
const InvestorDashboard = () => {
  ...
  const renderOverview = () => (
    <div className="space-y-6 pb-20 md:pb-6">
      ...
      <div className={`${cardBgClass} rounded-xl p-4 md:p-5 shadow-sm border ${borderClass} mb-6`}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-3">
            <div className={`w-3 h-3 rounded-full flex-shrink-0 ${tradingStatus === 'active' ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`}></div>
            <div>
              <h3 className={`font-semibold ${textClass} text-sm md:text-base`}>Trading Status</h3>
              <p className={`text-xs md:text-sm ${tradingStatus === 'active' ? 'text-green-600' : textSecondaryClass}`}>
                {tradingStatus === 'active' ? 'Active - Trading' : 'Inactive - Paused'}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 md:gap-3">
          <button
            ...
            className={`py-2.5 md:py-3 px-3 md:px-4 rounded-xl font-semibold shadow-md transition-all duration-300 border flex items-center justify-center space-x-1.5 md:space-x-2 text-sm md:text-base ${tradingStatus !== 'active' ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white border-emerald-500 hover:from-emerald-600 hover:to-emerald-700 shadow-emerald-200' : 'bg-white text-gray-400 border-slate-200 cursor-not-allowed'}`}
        ...
        <p className={`mt-2 text-xs ${textSecondaryClass} text-center`}>
          <Info className="w-3 h-3 inline mr-1" />
          Changes apply Sunday 00:01 (NY)
        </p>
      </div>
      ...
      <div className={`${cardBgClass} rounded-xl p-6 shadow-sm border ${borderClass}`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`px-3 py-1 rounded-full text-sm font-medium ${
            (userProfile.weeklyRisk || 2.5) <= 1.5 ? 'bg-green-100 text-green-700' :
